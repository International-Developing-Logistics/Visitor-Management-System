...
export async function sendVehicleRequestApprovalEmail({ request, approveUrl, rejectUrl, facilityLabel }) {
  ...
  const subject = `Vehicle request — ${request.employee_name}${facilityLabel ? ` (${facilityLabel})` : ""}`;
  const html = `
    <div style="font-family: -apple-system, Helvetica, Arial, sans-serif; color:#16211f; max-width:520px;">
      <p style="font-size:16px;">${request.employee_name} has requested a company vehicle${facilityLabel ? ` at ${facilityLabel}` : ""}.</p>
      <table style="width:100%; border-collapse:collapse; font-size:14px; margin-bottom:24px;">
